...
public class CrearCuentaAdmin extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_crear_cuenta_admin);

        //Menu de navegación
        BottomNavigationView bottomNavigationView = findViewById(R.id.bottomNav);

        bottomNavigationView.setSelectedItemId(R.id.btnCrearCuentaAdmin);
            ...
            public boolean onNavigationItemSelected(@NonNull MenuItem item) {
                switch (item.getItemId()){
                    ...
                    case R.id.crearcuenta:
                        startActivity(new Intent(getApplicationContext(), CrearCuentaAdmin.class));
                        overridePendingTransition(0,0);
                        return true;
                }
                return false;
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.GINGERBREAD) {
                if (!usuario.getNombre().isEmpty() && !usuario.getApellido().isEmpty() && !usuario.getCorreo().isEmpty() && !usuario.getPassword().isEmpty()) {
                    if (usuario.getPassword().length() >= 6) {
                        ...
                        Toast.makeText(CrearCuentaAdmin.this, "Error la contraseña debe tener mas de 6 caracteres", Toast.LENGTH_SHORT).show();
                    }
                } else {
                    Toast.makeText(CrearCuentaAdmin.this, "Error campo no puede estar vacío", Toast.LENGTH_SHORT).show();
                }
            }
        });
        ...
        mAuth.createUserWithEmailAndPassword(usuario.getCorreo(), usuario.getPassword()).addOnCompleteListener(new OnCompleteListener<AuthResult>() {
            ...
            public void onComplete(@NonNull Task<AuthResult> task) {
                if (task.isSuccessful()) {
                    ...
                    mDatabase.child("Users").child(id).setValue(usuario).addOnCompleteListener(new OnCompleteListener<Void>() {
                        ...
                        public void onComplete(@NonNull Task<Void> task1) {
                            if(task1.isSuccessful()) {
                                startActivity( new Intent(CrearCuentaAdmin.this,InicioSesion.class));
                                finish();
                            }
                        }
                    });
                    ...
                    Toast.makeText( CrearCuentaAdmin.this,"Este usuario ya está registrado", Toast.LENGTH_SHORT).show();
                }
            }
        });
    }
